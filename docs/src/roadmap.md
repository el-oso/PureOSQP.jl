...
| item | note |
|---|---|
| GPU arrays | `setup` rejects them at `validate`, which scalar-indexes. See below |
...
**GPU arrays.** The element type and the storage type both propagate correctly — buffers are
`similar` to the caller's — but roughly thirty loops in `admm.jl`, `termination.jl`,
`scaling.jl`, `rho.jl` and `types.jl` index elementwise, which `GPUArraysCore` forbids. The
shape of the fix: express the per-element work (the projection onto `[l, u]`, the relaxation,
`project_polar_reccone!`, the `ρ` classification) as scalar functions used by both a CPU loop
and a broadcast, and route GPU arrays to the matrix-free backend, which is the only one whose
factorization has a GPU counterpart — `bunchkaufman!` has none and CHOLMOD is CPU-only. Note
`JLArray <: StridedArray` is true, so a JLArrays test of a *direct* backend silently succeeds
through CPU LAPACK and proves nothing about CUDA; JLArrays is a sound gate for the
scalar-indexing discipline and for nothing else.
...
`linsys = :auto | :kkt | :indirect`, which selects a formulation; the library underneath is
whatever BLAS is loaded, so `using MKL` is the whole of the MKL story.
