...
"""
    ReducedCholesky{T,F} <: LinearSystem

Eliminates `ν` and factors the `n×n` symmetric positive definite reduced system
...
This is the default: it is smaller than the full system for every `m`, and measurement
puts it faster in every dense regime.
"""
mutable struct ReducedCholesky{T <: AbstractFloat, F} <: LinearSystem
    W::Matrix{T}
    R::Matrix{T}
    fact::F
end

function ReducedCholesky{T}(n::Integer, m::Integer) where {T <: AbstractFloat}
    fact = cholesky!(Symmetric(fill(one(T), 1, 1)))
    return ReducedCholesky{T, typeof(fact)}(Matrix{T}(undef, m, n), Matrix{T}(undef, n, n), fact)
end

"""
    FullKKT{T,F} <: LinearSystem
...
"""
mutable struct FullKKT{T <: AbstractFloat, F} <: LinearSystem
    K::Matrix{T}
    rhs::Vector{T}
    fact::F
end

function FullKKT{T}(n::Integer, m::Integer) where {T <: AbstractFloat}
    fact = bunchkaufman!(Symmetric(fill(one(T), 1, 1)))
    return FullKKT{T, typeof(fact)}(
        Matrix{T}(undef, n + m, n + m), Vector{T}(undef, n + m), fact
    )
end
...
function factorize!(ls::ReducedCholesky{T}, ws)::Bool where {T}
    ...
    F = cholesky!(Symmetric(ls.R); check = false)
...
function solve_system!(ls::ReducedCholesky, ws, rhs_x, rhs_z)::Nothing
    ...
    if m > 0
        ...
        for i in eachindex(ws.xtilde, ws.work_n)
            ws.xtilde[i] += ws.work_n[i]
        end
    end
