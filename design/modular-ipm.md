...
| S13 | **StrictMode + trim for IPM; Clarabel bench; docs.** Audit rows of §8.12; trim entries; `bench/ipm_vs_clarabel.jl`; iteration bounds; API/guarantees/algorithm pages. `SystemWeights` had no `@docs` entry: a comment between its docstring and the struct it documents silently dropped the docstring, breaking every page that referenced it; the comment moved inside the struct body. `verbose` output for the interior-point method is not built: §11 records it as a settled decision that the method has no per-iteration report, and `test/ipm_tests.jl` tests the refusal by name. Built as commit 822e29f. | audit green; trim green; bench committed; docs build clean | J |
